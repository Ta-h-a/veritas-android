...
const getTokenForKey = (key, value, styleProps, styleState) => {
  ...
  if (resolveAs === "none")
    return value;
  const { theme, conf = (0, import_config.getConfig)(), context, fontFamily, staticConfig } = styleState, themeValue = theme ? theme[value] || theme[value.slice(1)] : void 0, tokensParsed = conf.tokensParsed;
    ...
    if (value in conf.specificTokens)
      ...
      for (const cat in import_helpers.tokenCategories)
        if (key in import_helpers.tokenCategories[cat]) {
          ...
          res != null ? (valOrVar = res, hasSet = !0) : process.env.NODE_ENV === "development" && console.warn(
