...
function createComponent(staticConfig) {
  ...
  var component = /* @__PURE__ */ import_react.default.forwardRef(function(propsIn, forwardedRef) {
    ...
    var { context, isReactNative } = staticConfig, debugProp = propsIn.debug, styledContextValue = context ? import_react.default.useContext(context) : void 0, overriddenContextProps = null;
    ...
    var componentContext = import_react.default.useContext(import_ComponentContext.ComponentContext), groupContextParent = import_react.default.useContext(import_GroupContext.GroupContext), animationDriver = componentContext.animationDriver, useAnimations = animationDriver == null ? void 0 : animationDriver.useAnimations, componentState = (0, import_useComponentState.useComponentState)(props, animationDriver, staticConfig, config), { disabled, groupName, hasAnimationProp, hasEnterStyle, isAnimated, isExiting, isHydrated, presence, presenceState, setState, noClass, state, stateRef, supportsCSS, willBeAnimated, willBeAnimatedClient, startedUnhydrated } = componentState;
    ...
    if (!isPassthrough && (hasAnimationProp || groupName) && (animationDriver != null && animationDriver.avoidReRenders)) {
      ...
      var updateGroupListeners = updateGroupListeners2, _componentContext, ogSetStateShallow = setStateShallow;
      stateRef.current.updateStyleListener = function() {
        var updatedState = stateRef.current.nextState || state, mediaState2 = stateRef.current.nextMedia, nextStyles = (0, import_getSplitStyles.getSplitStyles)(props, staticConfig, theme, themeName, updatedState, mediaState2 ? {
          ...styleProps,
        ...
        if (!(next === prev || (0, import_is_equal_shallow.isEqualShallow)(prev, next))) {
          ...
          if (stateRef.current.nextState = updatedState, canAvoidReRender) {
            ...
            process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile" && console.info("[\u{1F40C}] re-render", {
              canAvoidReRender,
              next
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`), process.env.NODE_ENV === "development" && time && time`destructure`, !isPassthrough && groupContext && // avoids onLayout if we don't need it
    ...
    }, forwardedRef, import_setElementProps.setElementProps, animatedRef)), viewProps.ref = stateRef.current.composedRef, process.env.NODE_ENV === "development" && !isReactNative && !isText && import_constants.isWeb && !isHOC && import_react.default.Children.toArray(props.children).forEach(function(item) {
...
` && console.error(`Unexpected text node: ${item}. A text node cannot be a child of a <${staticConfig.componentName || propsIn.tag || "View"}>.`, props);
    ...
    process.env.NODE_ENV === "development" && import_constants.isWeb && (0, import_constants.useIsomorphicLayoutEffect)(function() {
      if (debugProp === "verbose") {
        ...
        var cssStyleDeclarationToObject = cssStyleDeclarationToObject2, computed = stateRef.current.host ? cssStyleDeclarationToObject2(getComputedStyle(stateRef.current.host)) : {};
      ...
      var tm;
      if (state.unmounted) {
        if (animationDriver != null && animationDriver.supportsCSS || import_constants.isAndroid)
    ...
    if (renderReset && ResetPresence && (content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResetPresence, {
      ...
      children: content
    })), process.env.NODE_ENV === "development" && time && time`group-context`, content = disableTheme || !splitStyles ? content : (0, import_Theme.getThemedChildren)(themeState, content, themeStateProps, !1, stateRef), process.env.NODE_ENV === "development" && time && time`themed-children`, staticConfig.context) {
    ...
    if (overriddenContextProps) {
      ...
      for (var key1 in styledContextValue)
        key1 in overriddenContextProps || (overriddenContextProps[key1] = styledContextValue[key1]);
    ...
    if (process.env.NODE_ENV === "development" && time && time`context-override`, 0)
      var styleTags;
  ...
  function styleable(Component2, options) {
    var _Component_render, skipForwardRef = import_constants.IS_REACT_19 && typeof Component2 == "function" && Component2.length === 1 || ((_Component_render = Component2.render) === null || _Component_render === void 0 ? void 0 : _Component_render.length) === 2, out = skipForwardRef ? Component2 : /* @__PURE__ */ import_react.default.forwardRef(Component2), extendedConfig = extendStyledConfig(options == null ? void 0 : options.staticConfig);
    return out = options != null && options.disableTheme ? out : (0, import_themeable.themeable)(out, extendedConfig, !0), (extendedConfig.memo || process.env.TAMAGUI_MEMOIZE_STYLEABLE) && (out = /* @__PURE__ */ import_react.default.memo(out)), out.staticConfig = extendedConfig, out.styleable = styleable, out;
  }
