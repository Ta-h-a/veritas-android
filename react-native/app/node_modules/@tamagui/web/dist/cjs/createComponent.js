...
function createComponent(staticConfig) {
  ...
  const component = import_react.default.forwardRef((propsIn, forwardedRef) => {
    ...
    if (styledContextValue || defaultProps) {
      const [nextProps, overrides] = (0, import_mergeProps.mergeComponentProps)(
        defaultProps,
        styledContextValue,
        propsIn
      );
    ...
    const componentContext = import_react.default.useContext(import_ComponentContext.ComponentContext), groupContextParent = import_react.default.useContext(import_GroupContext.GroupContext), animationDriver = componentContext.animationDriver, useAnimations = animationDriver?.useAnimations, componentState = (0, import_useComponentState.useComponentState)(
      props,
      animationDriver,
      staticConfig,
      config
    ...
    const hasTextAncestor = !!(import_constants.isWeb && isText && componentContext.inText), isTaggable = !Component || typeof Component == "string", tagProp = props.tag, element = import_constants.isWeb && isTaggable && tagProp || Component, BaseTextComponent = BaseText || element || "span", BaseViewComponent = BaseView || element || (hasTextAncestor ? "span" : "div");
    ...
    if ("themeInverse" in props && (themeStateProps.inverse = props.themeInverse), "theme" in props && (themeStateProps.name = props.theme), typeof stateRef.current.isListeningToTheme == "boolean" && (themeStateProps.needsUpdate = () => !!stateRef.current.isListeningToTheme), process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      ...
      if (console.info(
        ...
        (0, import_log.log)({ noClass, isAnimated, isWeb: import_constants.isWeb, supportsCSS });
      else {
        console.groupEnd();
        const ch = propsIn.children;
        let childLog = typeof ch == "string" ? ch.length > 4 ? ch.slice(0, 4) + "..." : ch : "";
        childLog.length && (childLog = `(children: ${childLog})`), console.groupCollapsed(`${childLog} [inspect props, state, context \u{1F447}]`), (0, import_log.log)("props in:", propsIn), (0, import_log.log)("final props:", props, Object.keys(props)), (0, import_log.log)({ state, staticConfig, elementType, themeStateProps }), (0, import_log.log)({
          context,
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`
    ...
    props.containerType !== "normal" && (nonTamaguiProps.onLayout = (0, import_helpers.composeEventHandlers)(
      ...
      animatedRef
    )), viewProps.ref = stateRef.current.composedRef, process.env.NODE_ENV === "development" && !isReactNative && !isText && import_constants.isWeb && !isHOC && import_react.default.Children.toArray(props.children).forEach((item) => {
...
` && console.error(
    ...
    process.env.NODE_ENV === "development" && import_constants.isWeb && (0, import_constants.useIsomorphicLayoutEffect)(() => {
      ...
      let tm;
      if (state.unmounted) {
        if (animationDriver?.supportsCSS || import_constants.isAndroid)
    ...
    )), "group" in props && (content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_GroupContext.GroupContext.Provider, { value: allGroupContexts, children: content })), process.env.NODE_ENV === "development" && time && time`group-context`, content = disableTheme || !splitStyles ? content : (0, import_Theme.getThemedChildren)(themeState, content, themeStateProps, !1, stateRef), process.env.NODE_ENV === "development" && time && time`themed-children`, isReactNative && !asChild && (content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      "span",
      {
    ...
    if (process.env.NODE_ENV === "development" && time && time`context-override`, startedUnhydrated) {
      const styleTags = (0, import_react.useMemo)(() => {
        if (!isPassthrough)
          return (0, import_wrapStyleTags.getStyleTags)(Object.values(splitStyles.rulesToInsert));
      }, []);
  ...
  function styleable(Component2, options) {
    let out = import_constants.IS_REACT_19 && typeof Component2 == "function" && Component2.length === 1 || Component2.render?.length === 2 ? Component2 : import_react.default.forwardRef(Component2);
