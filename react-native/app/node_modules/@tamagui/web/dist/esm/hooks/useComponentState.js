...
const useComponentState = (props, animationDriver, staticConfig, config) => {
  const isHydrated = useDidFinishSSR(), needsHydration = !useIsClientOnly(), [startedUnhydrated] = useState(needsHydration && !isHydrated), useAnimations = animationDriver?.useAnimations, { isHOC } = staticConfig, stateRef = useRef({}), hasAnimationProp = !!(!isHOC && "animation" in props || props.style && hasAnimatedStyleValue(props.style)), supportsCSS = animationDriver?.supportsCSS, curStateRef = stateRef.current;
  ...
  isWeb && hasAnimationThatNeedsHydrate && !staticConfig.isHOC && !isHydrated && (isAnimated = !1, curStateRef.willHydrate = !0), disabled !== state.disabled && (disabled && Object.assign(state, defaultComponentStateMounted), state.disabled = disabled, setState((_) => ({ ...state })));
